...
//公斤与磅互转 1磅(lb)=0.4535924公斤(kg)
func Test15() {
	for _, b := range os.Args[1:] {
		if f, err := strconv.ParseFloat(b, 64); err != nil {
			fmt.Fprintf(os.Stderr, "ERROR : %x\n", err)
			os.Exit(1)
		} else {
			l := Lb(f)
			k := Kg(f)
